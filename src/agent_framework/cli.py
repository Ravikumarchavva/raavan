...
Usage:
    agent-framework start          # start server on default port 8001
    agent-framework start --port 9000 --reload
    agent-framework stop           # stop a running server (via PID file)
    agent-framework status         # check if server is running
    agent-framework chat           # interactive CLI chat with default agent
    agent-framework chat --model gpt-4o-mini --no-tools
"""
...
def cmd_stop(args: argparse.Namespace) -> None:
    ...
    sig = signal.SIGKILL if (hasattr(args, "force") and args.force) else signal.SIGTERM
    try:
        os.kill(pid, sig)
    ...
    print(f"Process {pid} did not exit. Use `agent-framework stop --force` to kill it.")
    sys.exit(1)
...
def _load_mcp_tools(server_urls: list[str]) -> list:
    ...
    for url in server_urls:
        try:
            ...
            discovered = asyncio.run(client.discover_tools())
